      ...
      if(event.getSource() == entrar ){
         ...
         if(l.binaria(d.getUsuario(), Long.parseLong(login.getText())))
         {
            if(senha.getText().equals(d.getSenhaAt(l.getPos())))
            {
               ...
               usuarioLogado = new String[2]; //Controla as informações de CPF e tipo de usuário logado para posteriores consultas e restrições
               usuarioLogado[0] = login.getText(); // CPF
               usuarioLogado[1] = String.valueOf( d.getTipoAt( l.getPos() ) ); // Tipo de Usuário
               home.usuarioLogado = this.usuarioLogado;

                                             
               if( d.getTipoAt( l.getPos() ) == 0 )// se usuário logado for do tipo funcionário, desabilita funções das quais não tem permissão
               {
            ...
            else JOptionPane.showMessageDialog(null, msgErro);//("Usuário e/ou Senha inválido(s)");
         }
